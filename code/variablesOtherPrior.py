...
def get_all_cues(weight_matrix, outcome, per_domain=False):
    ...
    if per_domain:
        ...
        domain_cues = {
            "Segment cues: ": segments,
            "Syllable cues: ": syllables,
            "Context cues: ": context,
        }
        return domain_cues
...
def sum_domain_cues(weight_matrix, cues, outcome):
    ...
    """
    context = [
        np.absolute(weight_matrix.at[cue, outcome])
        for cue in cues["Context cues: "]
        if cue.startswith("c.")
    ]
...
def get_prior(weight_matrix, word_outcome, domain_specific=False):
    ...
    if domain_specific:
        ...
        cues = get_all_cues(
            weight_matrix=weight_matrix, outcome=outcome, per_domain=True
        )
        all_prior = sum_domain_cues(
            weight_matrix=weight_matrix, cues=cues, outcome=outcome
        )
...
def activation(
    word_outcome, event_files, weight_matrix, c1, c2=None, domain_specific=False
):
    ...
    if is_outcome(weight_matrix=weight_matrix, outcome=word_outcome):
        ...
        return (
            "The word " + word_outcome + " is not in the outcomes of the weight matrix."
        )

    # Get the syllable and segments cues for the outcome.
    cues = get_all_predicting_cues(
        event_files=event_files, outcome=outcome, per_domain=False, no_context=True
    )
    ...
    if domain_specific:
        ...
        all_activation = sum_domain_cues(
            weight_matrix=weight_matrix, cues=domain_cues, outcome=outcome
        )
...
def get_activation(
    weight_matrix, event_files, word_outcome, cues=None, domain_specific=False
):
    ...
    if domain_specific:
        if not cues:
            cues = get_predicting_cues(
                event_files=event_files, outcome=outcome, per_domain=True
            )
    ...
    # Get general activation
        ...
        if not cues:
            cues = get_predicting_cues(
                event_files=event_files, outcome=outcome, per_domain=False
            )
